...
SCHEMA_PATH = INPUT_DIR / 'import_loops' / 'main_schema.json'
...
def test_main_class_references_second(tmp_path: Path):
    ...
    assert 'SecondObjectInput' in content
...
def test_looping_reference_removed_from_third(tmp_path: Path):
    run_generate(SCHEMA_PATH, tmp_path)
    content = (tmp_path / 'third_object_input.py').read_text()

    assert 'class ThirdObjectInput' in content
    assert 'third_string' in content
    # The back-reference to SecondObjectInput should be removed
    assert 'SecondObjectInput' not in content
...
def test_no_circular_import(tmp_path: Path):
    ...
    # second imports third, but third must NOT import second
